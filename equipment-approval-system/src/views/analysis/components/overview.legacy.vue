...
const initHistoryChart = () => {
  ...
  if (hasDeviceFilter) {
    ...
    // 原有的横向柱状图
    const rows = isDrilling ? chartData.historyDrill : chartData.history
...
const loadHistoryDrilldown = async (level: 'city' | 'county' | 'institution') => {
  try {
    loading.value = true
    const data = await AnalysisApi.getHistoryDrilldown({
      level,
    ...
    chartData.historyDrill = normalizeHistoryDrillRows(
      data as any[],
      level === 'city' ? 'region' : level === 'county' ? 'county' : 'institutionName'
    )
    renderHistoryChart()
  } catch (err) {
    console.log(err)
