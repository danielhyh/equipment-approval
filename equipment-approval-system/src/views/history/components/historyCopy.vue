<template>
  <div class="content-msg-page" ref="contentMsgPageRef">
    ...
    <!-- <License v-if="hasData" v-bind="licenseProps" ref="licenseRef" /> -->
  </div>
</template>

<script setup lang="ts" name="HistoryCopy">
import License from '@/views/Processing/components/license.vue'

// 日期格式化工具函数
const dayTimeFormate = (time: string | undefined) => {
  if (!time) return ''
...
const formateDialogLicense = (data: any) => {
  let arr: any[] = []
  ...
  licenseProps.value.code = data.code || data.licenseNumber
  ...
  (newVal) => {
    if (!!newVal && Object.keys(newVal).length > 0) {
      basicMsg.value.forEach((item) => {
        ...
        basicData.value[item.key] = newVal[item.key]
      })
      formateDialogLicense(newVal)
      hasData.value = true
      nextTick(() => {
        licenseDomResize()
      })
