<template>
  <div :style="{ width: `${width}px`, maxHeight: '600px' }" class="process-panel__container">
    <el-collapse v-if="isReady" v-model="activeTab">
      ...
      <el-collapse-item v-if="isTaskCollapseItemShow(elementType)" key="task" name="task">
        <template #title
      ...
      <el-collapse-item key="customConfig" name="customConfig">
      ...
      <el-collapse-item v-if="elementType === 'IntermediateCatchEvent'" name="timeEvent">
        ...
        <TimeEventConfig :key="elementId" :businessObject="bpmnElement.value?.businessObject" />
      </el-collapse-item>
    </el-collapse>
  </div>
</template>
