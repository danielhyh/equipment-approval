<template>
  <Dialog v-model="dialogVisible" title="选择链接" width="65%">
    <div class="h-500px flex gap-8px">
      <!-- 左侧分组列表 -->
      <el-scrollbar ref="groupScrollbar" view-class="flex flex-col" wrap-class="h-full">
        <el-button
      ...
      <!-- 右侧链接列表 -->
      <el-scrollbar ref="linkScrollbar" class="h-full flex-1" @scroll="handleScroll">
        <div v-for="(group, groupIndex) in APP_LINK_GROUP_LIST" :key="groupIndex">
          <!-- 分组标题 -->
          <div ref="groupTitleRefs" class="font-bold">{{ group.name }}</div>
          <!-- 链接列表 -->
          <el-tooltip
